...
public final class FloodEventSystem {
    ...
    private static final ConcurrentHashMap<ResourceKey<Level>, Long> NEXT_NATURAL_ROLL = new ConcurrentHashMap<>();
    ...
    public static void onLevelUnload(ServerLevel level) {
        ACTIVE_FLOODS.remove(level.dimension());
        NEXT_NATURAL_ROLL.remove(level.dimension());
    }
    ...
    private static void tryStartNaturalFlood(ServerLevel level) {
        if (!FlowingFluids.config.enableMod
                || !FlowingFluids.config.enableFloodEvents
                || FlowingFluids.config.isDimensionExcluded(level)
                || FlowingFluids.config.floodStartChancePerDay <= 0.0f) {
            return;
        }

        long now = level.getGameTime();
        long nextRoll = NEXT_NATURAL_ROLL.getOrDefault(level.dimension(), 0L);
        if (now < nextRoll) {
            return;
        }

        if (FlowingFluids.config.floodRequiresRain && !level.isRaining()) {
            NEXT_NATURAL_ROLL.put(level.dimension(), now + 1200L);
            return;
        }

        NEXT_NATURAL_ROLL.put(level.dimension(), now + 24000L);
        float chance = FlowingFluids.config.floodStartChancePerDay;
        if (level.isThundering()) {
            chance *= Math.max(0.1f, FlowingFluids.config.floodThunderstormChanceMultiplier);
        }
        if (level.random.nextFloat() > Mth.clamp(chance, 0.0f, 1.0f)) {
            return;
        }

        BlockPos center = chooseNaturalFloodCenter(level);
        int surfaceY = level.getHeight(Heightmap.Types.WORLD_SURFACE, center.getX(), center.getZ()) - 1;
        BlockPos floodCenter = new BlockPos(center.getX(), Math.max(level.getMinBuildHeight(), surfaceY), center.getZ());
        if (startFlood(level, floodCenter, FlowingFluids.config.floodDefaultRadius,
                FlowingFluids.config.floodDefaultDurationTicks, Integer.MIN_VALUE)
                && FlowingFluids.config.announceFloodEvents) {
            level.getServer().getPlayerList().broadcastSystemMessage(Component.literal(
                    "Flood waters are rising near " + floodCenter.getX() + ", " + floodCenter.getY() + ", " + floodCenter.getZ()
                            + ". Low ground and shorelines will fill while the storm holds."), false);
        }
    }

    private static BlockPos chooseNaturalFloodCenter(ServerLevel level) {
        if (!level.players().isEmpty()) {
            ServerPlayer player = level.players().get(level.random.nextInt(level.players().size()));
            int radius = Math.max(16, FlowingFluids.config.floodDefaultRadius);
            int dx = level.random.nextInt(radius * 2 + 1) - radius;
            int dz = level.random.nextInt(radius * 2 + 1) - radius;
            return player.blockPosition().offset(dx, 0, dz);
        }
    ...
    private static void placeFloodWater(ServerLevel level, BlockPos pos, int amount) {
        ...
        FLUIDS_API.placeFluidAmountFromPos(level, pos, Fluids.WATER, clampedAmount, false, true);
    }
