...
public final class FluidAutoTickDelay {
    private static final float DECREASE_DELAY_MARGIN_MS = 15.0f;
    private static final float INCREASE_DELAY_MARGIN_MS = 5.0f;
    ...
    public static void onServerTick(MinecraftServer server) {
        ...
        float marginMs = lastTargetMspt - lastMeasuredMspt;

        waterExtraDelay = ff$adjustDelayWithHysteresis(
                previousWaterDelay,
                FlowingFluids.config.autoTickDelayWaterMaxExtraDelay,
                marginMs);
        lavaExtraDelay = ff$adjustDelayWithHysteresis(
                previousLavaDelay,
    ...
    private static int ff$adjustDelayWithHysteresis(int current, int maxExtraDelay, float marginMs) {
        if (maxExtraDelay <= 0) {
            return 0;
        }
        if (marginMs <= INCREASE_DELAY_MARGIN_MS) {
            return Mth.clamp(current + 1, 0, maxExtraDelay);
        }
        if (marginMs >= DECREASE_DELAY_MARGIN_MS) {
            return Mth.clamp(current - 1, 0, maxExtraDelay);
        }
        return Mth.clamp(current, 0, maxExtraDelay);
    }
